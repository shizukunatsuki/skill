...
The resources in `references/external-resources.md` have already been checked, so starting from them saves the research. **`references/external-resources.md` is a reference, not a whitelist.** A resource outside `references/external-resources.md` may fit a given document better, and choosing one is fine; what is fixed is the rules in "How to use `references/external-resources.md`", not the list of entries.

Read `references/external-resources.md` while planning, before deciding which figures the document will have, because knowing what "Libraries" covers, and what it does not, is what shows early which figures have a library behind them and which will need one found beyond `references/external-resources.md` or asked about. Read `references/external-resources.md` again before adding a figure to a document that already exists. Every HTML document loads typefaces, including one with no figures at all, so "Typefaces" applies even when no library is needed.

### Rules for any resource

- **Start from the relevant entry in `references/external-resources.md`.** If an entry fits, use it and skip the search.
- **Verify every URL by requesting it** before it goes into the document — scripts, stylesheets and font endpoints alike. Request font stylesheets with a browser User-Agent: Google Fonts answers a plain request with a different stylesheet (TrueType sources) from the one a browser gets (WOFF2 sources), so a check made without a browser User-Agent is not a check of what the reader will receive.
- **Report every external resource the document loads**, libraries and typefaces both, with its version where it has one and what it is for. Mark any resource that did not come from `references/external-resources.md`, give the reason it was chosen, and put a proposed entry for it in the reply, written in the shape "Entry format" sets out, for the user to add: `references/external-resources.md` is read-only guidance that no kind of turn edits, as "The skill's own files are read-only guidance" in SKILL.md states.
- **If nothing suitable exists for a figure, ask the user** rather than building the missing component or silently producing a weaker result. Ask about the figure alone, not about the document: everything else is delivered, with a marked gap where the figure belongs. A single unresolved figure never holds up the rest of the document.
- **Load nothing the document does not use.** A page with no formulas loads no math renderer. Typefaces carry a requirement of their own: every HTML document loads the faces it sets text in, rather than naming local families and inheriting whatever the reader has installed.

### Additional rules for libraries

- **A library must be a large, actively maintained project with a CDN build.** Be wary of low-level toolkits: reaching for one usually means writing the layout, geometry and interaction by hand, which is the work the lecture-notes skill exists to avoid. The caution about low-level toolkits is a reason to prefer a higher-level library, not a ban.
- **Resolve the version yourself.** Query the registry — `https://data.jsdelivr.com/v1/packages/npm/<package>` reports the current `latest` — then request the exact file before writing its URL into the document, and pin that exact version. The versions recorded in `references/external-resources.md` are a starting point for resolving the version, not a substitute for resolving it.
- **Check the API against the version loaded**, in that version's documentation or source. Do not write library calls from memory. Item 15 in SKILL.md asks for the API check as well, and doing the API check carefully is what keeps a wrong call out, because at generation time a wrong call surfaces only where the code path actually runs: during SKILL.md's one-load check, or during item 13's pass over the native controls. A wrong call inside a drag handler or a canvas-drawn control's callback runs during neither the one-load check nor item 13's pass, and goes unnoticed until the runtime round.
...
SKILL.md allows two arrangements: synchronous loading, where libraries load with plain synchronous script tags; and the arrangement where every library-calling script waits for `DOMContentLoaded` — the skeleton's shared script where it makes such a call, as the `mermaid.initialize` call in the Mermaid entry does, each section's, and the final pass's. The entries in `references/external-resources.md` assume synchronous loading, which is the simpler arrangement. Where every library-calling script waits for `DOMContentLoaded`, `defer` is also safe, since a deferred script runs before `DOMContentLoaded` fires. The final pass has to wait too: it sits at the end of the body and looks late enough, and is not, since an inline script runs when the parser reaches it and a deferred library runs only after parsing ends — `renderMathInElement(document.body)` called plainly from the final pass under a deferred KaTeX fails with the function undefined. `async` is safe under neither arrangement, since an `async` library can run after `DOMContentLoaded` has fired, and an ES-module build fits neither arrangement, since what a module exports is not a global a section's script can call. Four rules apply under synchronous loading; "Drop `defer` and `async`" and "Do not use an ES-module build" exist because library documentation often shows what synchronous loading does not allow:

- **Drop `defer` and `async`.** The MathJax package's own README, for one, loads MathJax with `defer`; under synchronous loading the `defer` goes.
- **Do not use an ES-module build.** A `<script type="module">` is deferred by default even with no attribute on it, so a module script breaks synchronous loading silently. Use the classic build each entry lists, which exposes a global.
...
- **Some libraries finish their work asynchronously even when their script loads synchronously.** MathJax typesets after it starts up, Mermaid renders its diagrams on page load, and OpenSheetMusicDisplay loads a score before it can draw it. Every entry's **Loading** bullet states whether the entry's library finishes asynchronously and what signal it gives, which makes the **Loading** bullet the place to look for any particular library. A section's inline script must not read what an asynchronously finishing library produces — the size of a typeset formula, a rendered diagram, an engraved score — while the page is still parsing. Wait for the signal the library's entry names. SKILL.md's one-load check waits for the signals the entries name before judging whether content rendered.
...
- **Files** — every file the page loads for the library, as full CDN URLs, in load order.
- **Global** — the name a section's script calls.
- **Loading** — how the library behaves under synchronous loading, including any signal to wait for.
- **Use it for** — what the figure has to do for the library to be the right choice, and what sends the choice elsewhere.
- **Pitfalls** — failures found in checking that are not obvious from the library's documentation.

A **typeface** entry is a row in the role table under "Typefaces", together with the record of where it was verified: for Google Fonts, a URL in the list of verified stylesheets under "Typefaces"; for Fontsource, the package with its pinned version.
